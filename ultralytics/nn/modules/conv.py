...
class GSConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, d=1, act=True):
        #g:分组卷积的分组数，d:空洞卷积的扩张系数，act：是否使用激活函数
        super().__init__()
        c_ = c2 // 2#输出通道数的一半，用于构建两个卷积操作
        self.cv1 = Conv(c1, c_, k, s, None, g, d, act)
        #第一个卷积层，使用 c1 输入通道和 c_ 输出通道。卷积核大小、步长等参数由 k, s, g, d 控制。
        self.cv2 = Conv(c_, c_, 5, 1, None, c_, d, act)
        #第二个卷积层，输入和输出通道均为 c_，卷积核大小为 5x5，步长为 1。

    def forward(self, x):
        x1 = self.cv1(x)
        #对输入 x 应用第一个卷积层 self.cv1
        x2 = torch.cat((x1, self.cv2(x1)), 1)
        #对 x1 应用第二个卷积层 self.cv2，然后将 x1 和 self.cv2(x1) 在通道维度上拼接。
        # shuffle
        b, n, h, w = x2.data.size()
        #获取 x2 的批量大小 b、通道数 n、高度 h 和宽度 w。
        b_n = b * n // 2
        #计算新的批量大小 b_n
        y = x2.reshape(b_n, 2, h * w)
        #调整 x2 的形状，将其变为 (b_n, 2, h * w)。
        y = y.permute(1, 0, 2)# 对维度进行转换
        y = y.reshape(2, -1, n // 2, h, w)#将其重新调整为 (2, -1, n // 2, h, w) 形状
        return torch.cat((y[0], y[1]), 1)#y[0] 和 y[1] 在通道维度上拼接
...
class GSBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):
        ...
        self.conv_lighting = nn.Sequential(
            GSConv(c1, c_, 1, 1),#将输入通道数降低到c_
            GSConv(c_, c2, 1, 1, act=False))#将通道数提升到c2，并不应用激活函数
        # for receptive field
        self.conv = nn.Sequential(
            GSConv(c1, c_, 3, 1),
            GSConv(c_, c2, 3, 1, act=False))
        self.shortcut = Conv(c1, c2, 3, 1, act=False)
        #用于实现残差连接。它对输入进行一次 3x3 卷积操作，将通道数从 c1 提升到 c2，并且不应用激活函数 (act=False)
...
class VoVGSCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        #n：GSBottleneck 的重复次数，默认为 1，shortcut: 是否使用残差连接（在这个实现中没有直接使用），默认为 True。
        #g分组卷积的分组数（在这个实现中没有直接使用），默认为 1。e：压缩比，决定了瓶颈层的通道数比例，默认为 0.5
        super().__init__()
        c_ = int(c2 * e)
        #计算瓶颈层的通道数 c_，即 c2 的 e 倍。
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(2 * c_, c2, 1)
        self.m = nn.Sequential(*(GSBottleneck(c_, c_) for _ in range(n)))
        #一个包含 n 个 GSBottleneck 层的 nn.Sequential 容器，
        # 每个 GSBottleneck 层处理 c_ 个通道。n 决定了 GSBottleneck 的堆叠次数。
